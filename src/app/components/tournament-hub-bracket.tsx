...
import { Trophy, Swords, Loader2, GitBranch, AlertCircle, Crown, Volume2, Trash2, Play, CheckCircle } from 'lucide-react';
  ...
  if (!bracket) {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
        ...
        <p className="font-bold text-lg text-foreground">No bracket yet</p>
        <p className="text-sm mt-1 text-center max-w-md">
          {isOwner
            ? canGenerate
              ? 'All team rosters are finalized. Generate the bracket to seed teams by average rank.'
              : 'Advance to Roster Lock to generate the bracket once all teams and ranks are set.'
            : 'The bracket will appear here once it has been generated by the tournament organizer.'}
        </p>
        {canGenerate && onGenerateBracket && (
          <button
            onClick={onGenerateBracket}
            disabled={generating}
            className="mt-4 flex items-center gap-2 px-5 py-2.5 rounded-xl bg-harvest text-white font-bold text-sm hover:bg-harvest/90 transition-all disabled:opacity-50"
          >
