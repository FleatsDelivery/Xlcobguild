  ...
  return (
    <div className="bg-white rounded-2xl border-2 border-field-dark/10 p-6">
      <div className="flex items-center gap-3 mb-6">
        <Flame className="w-6 h-6 text-harvest" />
        <h2 className="text-2xl font-bold text-field-dark">Popular Heroes</h2>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {heroStats.map((stat, index) => {
          ...
          return (
            <div
              key={stat.hero_id}
              className="flex items-center gap-4 p-4 bg-silk rounded-xl border-2 border-field-dark/10 hover:border-harvest transition-all"
            >
              <div className="relative shrink-0">
                <img
                  src={heroImage}
                  alt={heroName}
                  className="w-24 h-14 rounded-lg border-2 border-field-dark/10 object-cover"
                />
                <span className="absolute -top-2 -left-2 w-6 h-6 bg-harvest text-white rounded-full flex items-center justify-center text-xs font-bold">
                  {index + 1}
                </span>
              </div>
              <div className="flex-1">
                <h3 className="font-bold text-field-dark">{heroName}</h3>
