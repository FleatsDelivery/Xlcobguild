  ...
  if (players.length === 0) {
    return (
      <div className="bg-white rounded-2xl border-2 border-[#0f172a]/10 p-12 text-center">
        <User className="w-16 h-16 mx-auto mb-4 text-[#0f172a]/20" />
        <p className="text-[#0f172a]/60">No player profiles available yet</p>
      </div>
    );
  }
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      {players.map((player) => {
        ...
        return (
          <div
            key={player.steam_id}
            className="bg-white rounded-xl border-2 border-[#0f172a]/10 overflow-hidden hover:border-[#f97316] transition-all hover:shadow-lg"
          >
            {/* Player Header */}
            <div className="bg-gradient-to-br from-[#f97316] to-[#ea580c] p-6 text-white relative">
            ...
            {player.stats && (
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  <div className="bg-[#fdf5e9] rounded-lg p-3">
                    <div className="flex items-center gap-1 mb-1">
                      <Trophy className="w-3 h-3 text-[#f97316]" />
                      <span className="text-xs font-semibold text-[#0f172a]/60">Win Rate</span>
                    </div>
                    <p className="text-xl font-black text-[#f97316]">{winRate}%</p>
                  </div>
                  <div className="bg-[#fdf5e9] rounded-lg p-3">
                    <div className="flex items-center gap-1 mb-1">
                      <Target className="w-3 h-3 text-[#f97316]" />
                      <span className="text-xs font-semibold text-[#0f172a]/60">KDA</span>
                    </div>
                    <p className="text-xl font-black text-[#f97316]">{kda}</p>
                  </div>
                </div>

                <div className="bg-[#fdf5e9] rounded-lg p-3">
                  <p className="text-xs font-semibold text-[#0f172a]/60 mb-2">Tournament Stats</p>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <p className="text-sm font-black text-[#0f172a]">{player.stats.total_games}</p>
                      <p className="text-xs text-[#0f172a]/60">Games</p>
                    </div>
                    <div>
                      <p className="text-sm font-black text-green-600">{player.stats.wins}</p>
                      <p className="text-xs text-[#0f172a]/60">Wins</p>
                    </div>
                    <div>
                      <p className="text-sm font-black text-red-600">{player.stats.total_games - player.stats.wins}</p>
                      <p className="text-xs text-[#0f172a]/60">Losses</p>
                    </div>
                  </div>
                </div>

                <div className="bg-[#fdf5e9] rounded-lg p-3">
                  <p className="text-xs font-semibold text-[#0f172a]/60 mb-2">K / D / A</p>
            ...
            {/* Player Links */}
            <div className="border-t-2 border-[#0f172a]/10 p-3 flex gap-2">
              {player.dotabuff_url && (
                <a
